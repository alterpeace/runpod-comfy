...
Usage:
    ...
    python scripts/mock_runpod_server.py
...
async def submit_job(endpoint_id: str, request: Request):
    ...
    try:
        ...
        raise HTTPException(
            status_code=503,
            detail=f"Cannot connect to ComfyUI at {comfyui_url}. Is Docker running? ./scripts/run_local.sh"
        )
    except Exception as e:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Setup:
  1. Start local ComfyUI:  ./scripts/run_local.sh
  2. Start mock server:    python scripts/mock_runpod_server.py
...
Or use the one-command script:
  ./scripts/run_frontend.sh --mock
    ...
    try:
        ...
        logger.warning("  Start it with: ./scripts/run_local.sh")
