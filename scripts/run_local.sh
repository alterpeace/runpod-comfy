...
#
# Usage:
#   ./scripts/run_local.sh          # start (detached)
...
if [ "$NEEDS_PATCH" = true ]; then
    ...
    bash "$SCRIPT_DIR/fix_local_gpu_cdi.sh" >/dev/null 2>&1 || true
fi

# Step 2: Ensure GPU compose override exists
if [ ! -f "$GPU_OVERRIDE" ]; then
    echo "[run_local] Generating GPU compose override..."
    bash "$SCRIPT_DIR/gen_gpu_compose_override.sh" >/dev/null 2>&1
fi
...
" 2>&1; then
    ...
    echo "[run_local] ⚠️  GPU not detected — try: ./scripts/setup_local_gpu.sh"
fi
