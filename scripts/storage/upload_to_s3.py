...
S3 env vars (RUNPOD_S3_BUCKET, …) used by scripts/storage/sync_to_runpod_s3.sh.

Usage
-----
    # Upload a file, auto-generating the object key under the prefix:
    python scripts/storage/upload_to_s3.py path/to/video.mp4

    # Specify an explicit S3 key (path inside the bucket):
    python scripts/storage/upload_to_s3.py video.mp4 --key outputs/2026/my-video.mp4

    # Use the RunPod S3 backend instead of the general S3 env vars:
    python scripts/storage/upload_to_s3.py video.mp4 --runpod

    # Make the object publicly readable and print a public URL:
    python scripts/storage/upload_to_s3.py image.png --public

    # Generate a presigned download URL after uploading (private objects):
    python scripts/storage/upload_to_s3.py video.mp4 --presign 3600

    # Override the bucket/prefix from the command line:
    python scripts/storage/upload_to_s3.py video.mp4 --bucket my-bucket --prefix uploads
