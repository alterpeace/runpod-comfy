...
Usage:
  ...
  .venv/bin/python scripts/storage/upload_to_runpod.py video.mp4

  # Upload multiple files
  .venv/bin/python scripts/storage/upload_to_runpod.py video1.mp4 video2.mp4 image.png

  # Upload to a subfolder
  .venv/bin/python scripts/storage/upload_to_runpod.py video.mp4 --subfolder upscale_test

  # List files on the volume (no upload)
  .venv/bin/python scripts/storage/upload_to_runpod.py --list

  # Specify GPU type for the temp pod (default: cheapest)
  .venv/bin/python scripts/storage/upload_to_runpod.py video.mp4 --gpu "NVIDIA RTX A4000"

  # Keep the pod running after upload (for SSH access)
  .venv/bin/python scripts/storage/upload_to_runpod.py video.mp4 --keep-pod
  ...
  %(prog)s video.mp4 --keep-pod
    ...
    try:
        ...
        if args.keep_pod:
            ...
            log_warn(f"  Terminate with: .venv/bin/python scripts/storage/upload_to_runpod.py --list  # then terminate manually")
