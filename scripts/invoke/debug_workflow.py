...
Usage:
    ...
    python scripts/invoke/debug_workflow.py --workflow examples/text_to_image_simple.json --dry-run

    # Test against local ComfyUI
    python scripts/invoke/debug_workflow.py --target local --workflow examples/text_to_image_simple.json

    # Test against RunPod serverless endpoint
    python scripts/invoke/debug_workflow.py --target runpod --workflow examples/text_to_image_simple.json --wait

    # Override specific node inputs
    python scripts/invoke/debug_workflow.py --target runpod \\
    ...
    python scripts/invoke/debug_workflow.py --target runpod --workflow workflow.json --output-dir ./debug-output/

    # Print node graph
    python scripts/invoke/debug_workflow.py --workflow workflow.json --print-graph
"""
  ...
  python scripts/invoke/debug_workflow.py --workflow workflow.json --print-graph
    ...
    if args.target == "local":
        try:
            ...
            print(f"  Is ComfyUI running? Start with: ./scripts/build/run_local.sh")
            sys.exit(1)
        except Exception as e:
            print(f"\n{C.RED}ERROR: {e}{C.NC}")
            sys.exit(1)
