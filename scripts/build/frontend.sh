...
if [ "$MODE" = "auto" ]; then
    ...
    if curl -s http://127.0.0.1:8188/system_stats >/dev/null 2>&1; then
        ...
        if [ -n "${RUNPOD_ENDPOINT_ID:-}" ]; then
            ...
            echo -e "  1. Start local ComfyUI: ./scripts/build/run_local.sh"
...
if [ "$MODE" = "serverless" ] || [ "$MODE" = "mock" ]; then
    if [ ! -f "$CACHE_PATH" ]; then
        ...
        if curl -s http://127.0.0.1:8188/system_stats >/dev/null 2>&1; then
            ...
            echo -e "${YELLOW}  Start local ComfyUI first: ./scripts/build/run_local.sh${NC}"
            echo -e "${YELLOW}  Then: python scripts/build/fetch_object_info.py --source local${NC}"
...
if [ "$MODE" = "mock" ]; then
    ...
    if ! curl -s http://127.0.0.1:8188/system_stats >/dev/null 2>&1; then
        ...
        echo -e "${YELLOW}  Start it first: ./scripts/build/run_local.sh${NC}"
        exit 1
    fi
