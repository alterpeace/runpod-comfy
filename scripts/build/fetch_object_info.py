...
Usage:
    ...
    python scripts/build/fetch_object_info.py --source local

    # Fetch from local ComfyUI on a custom URL
    python scripts/build/fetch_object_info.py --source local --url http://localhost:9000

    # Fetch from RunPod serverless endpoint (cold-starts a worker)
    python scripts/build/fetch_object_info.py --source runpod --endpoint-id YOUR_ID

    # Refresh only model lists from network volume (no worker cold-start)
    python scripts/build/fetch_object_info.py --models-only --volume-id v1abc123

    # Specify output path (default: config/object_info_cache.json)
    python scripts/build/fetch_object_info.py --source local --output config/object_info_custom.json
"""
...
def fetch_from_local(url: str) -> dict:
    ...
    try:
        ...
        print("  Is ComfyUI running? Start it with: ./scripts/build/run_local.sh")
        sys.exit(1)
    ...
    while time.time() - start < max_wait:
        try:
            ...
            if status == "COMPLETED":
                ...
                print("    1. ./scripts/build/run_local.sh")
                print("    2. python scripts/build/fetch_object_info.py --source local")
                print()
                print("  OR: If you have a Cloudflare tunnel or SSH access to the worker,")
                print("  fetch from that URL:")
                print(f"    python scripts/build/fetch_object_info.py --source local --url <tunnel-url>")
                sys.exit(1)
...
def refresh_models_only(volume_id: str, api_key: str, cache_path: Path) -> dict:
    ...
    if not cache_path.exists():
        ...
        print("  Run first: python scripts/build/fetch_object_info.py --source local")
        sys.exit(1)
    ...
    print("    python scripts/build/fetch_object_info.py --source local")
    print()
