...
Usage:
    ...
    uv run python scripts/diag/fix_input_symlink.py            # dry-run first
    uv run python scripts/diag/fix_input_symlink.py --apply     # actually create symlinks
"""
import runpod
import os
import json
...
if apply and output.get("status") == "success":
    ...
    print("   uv run python scripts/diag/fix_input_symlink.py --apply")
else:
