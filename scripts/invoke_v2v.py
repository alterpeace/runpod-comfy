...
Usage:
  # Single video
  uv run python scripts/invoke_v2v.py --video rhizome.mp4

  # Video in a subfolder (relative to /runpod-volume/input/)
  uv run python scripts/invoke_v2v.py --video sample/clip_001.mp4

  # Batch all mp4s in a directory on the volume
  uv run python scripts/invoke_v2v.py --video-dir sample/
  ...
  # Override endpoint
  uv run python scripts/invoke_v2v.py --video rhizome.mp4 --endpoint-id abc123
"""
...
def main():
    ...
    if args.video:
        ...
        if not videos:
            ...
            print(f"Upload with: uv run python scripts/upload_to_runpod.py /path/to/videos --subfolder {args.video_dir}")
            sys.exit(1)
