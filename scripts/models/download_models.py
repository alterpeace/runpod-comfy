#!/usr/bin/env python3
"""Download LTX models/LoRAs listed in a model manifest JSON.

Supports both LTX-2.3 and LTX-2.5 manifests. By default uses LTX-2.5;
pass --version 23 (or --manifest config/ltx-2.3-models.json) for 2.3.

Usage:
    # LTX-2.5 (default)
    python scripts/models/download_models.py --profile low_vram_8gb
    python scripts/models/download_models.py --ids checkpoint_dev_int8 distilled_lora
    python scripts/models/download_models.py --profile full --dry-run
    python scripts/models/download_models.py --list

    # LTX-2.3
    python scripts/models/download_models.py --version 23 --profile low_vram_8gb
    python scripts/models/download_models.py --version 23 --list

    # Explicit manifest path
    python scripts/models/download_models.py --manifest config/ltx-2.3-models.json --list
...
_MANIFESTS = {
    "23": _PROJECT_ROOT / "config" / "ltx-2.3-models.json",
    "25": _PROJECT_ROOT / "config" / "ltx-2.5-models.json",
}
DEFAULT_MANIFEST = _MANIFESTS["25"]
...
def main() -> int:
    ...
    parser.add_argument("--version", choices=["23", "25"], default="25", help="LTX model version (default: 25)")
    parser.add_argument("--manifest", type=Path, default=None, help="Path to model manifest JSON (overrides --version)")
    ...
    manifest_path = args.manifest if args.manifest else _MANIFESTS[args.version]
