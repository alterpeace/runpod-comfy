#!/bin/bash
# Install everything needed to run LTX-2.3 V2V / IC-LoRA workflows:
#   1. ComfyUI-LTXVideo custom nodes (IC-LoRA guide/loader nodes)
#   2. ComfyUI-GGUF custom nodes (for low-VRAM quantized checkpoints)
#   3. Models/LoRAs from config/ltx-2.3-models.json, per a VRAM profile
#
# Usage:
#   ./scripts/models/install_ltx23.sh                     # profile=mid_vram_12_24gb (default)
#   ./scripts/models/install_ltx23.sh --profile low_vram_8gb
#   ./scripts/models/install_ltx23.sh --profile full
#   ./scripts/models/install_ltx23.sh --ids checkpoint_fp8 distilled_lora iclora_ingredients
#   ./scripts/models/install_ltx23.sh --dry-run
#   ./scripts/models/install_ltx23.sh --skip-nodes         # models only
#   ./scripts/models/install_ltx23.sh --skip-models        # nodes only
#
# Environment:
#   HF_TOKEN            Required for gated LoRA repos (Lightricks IC-LoRAs).
#                        Get one at https://huggingface.co/settings/tokens
#                        and click "Agree and Access" on each gated repo page.
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --force)
            FORCE=true; shift ;;
        --list)
            python3 "$SCRIPT_DIR/download_ltx23_models.py" --manifest "$PROJECT_ROOT/config/ltx-2.3-models.json" --list
            exit 0 ;;
        -h|--help)
            usage ;;
        *)
...
if [ "$SKIP_MODELS" = false ]; then
    log_info "=== Downloading models (profile: $PROFILE) ==="
    ...
    PY_ARGS=(--manifest "$PROJECT_ROOT/config/ltx-2.3-models.json" --output-dir "$MODELS_DIR")
    if [ ${#IDS[@]} -gt 0 ]; then
        PY_ARGS+=(--ids "${IDS[@]}")
    else
    ...
    python3 "$SCRIPT_DIR/download_ltx23_models.py" "${PY_ARGS[@]}"
fi

log_success "LTX-2.3 install complete."
